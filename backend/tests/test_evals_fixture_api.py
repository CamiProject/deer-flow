...
def test_fixture_trials_are_isolated_and_domain_writes_are_idempotent():
    ...
    with TestClient(app) as client:
        ...
        first = client.patch(
            "/api/internal/sites/site-1/display-name",
    ...
    assert changed.json()["state"]["sites"]["site-1"]["display_name"] == "New Display Name"
    assert changed.json()["state"]["sites"]["site-1"]["version"] == "2"
    assert changed.json()["unexpected_changes"] == []
    assert untouched.json()["state"]["sites"]["site-1"]["display_name"] == "Old Display Name"
...
def test_fixture_reports_state_changes_not_declared_by_case_expectation():
    ...
    assert state.json()["unexpected_changes"] == [
        "sites.site-1.display_name",
        "sites.site-1.version",
    ]
...
def test_fixture_iam_revalidation_issues_fresh_action_worker_token_and_can_deny():
    ...
    assert claims["sub"] == "eval-user-1"
    assert claims["scope"]["site_ids"] == ["site-1"]
