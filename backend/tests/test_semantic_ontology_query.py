    ...
    return SqlScopePolicyRegistry.from_mapping(
        {
            "version": "3",
            "tables": {
                "iot_site": {"access": "scoped", "scope_dimension": "site", "scope_column": "id"},
                "iot_project": {"access": "scoped", "scope_dimension": "site", "scope_column": "site_id"},
            },
        }
    )
...
def test_object_query_compiles_through_scope_guard(registry, policy, authorization):
    ...
    assert compiled.scoped_query.parameters == {"filter_0": "A", "scope_0_0": "site-1"}
...
def test_metric_query_allows_only_declared_dimensions_and_filters(registry, policy, authorization):
    compiled = compile_metric_query(
        ...
        allowed_databases=("tenant_db",),
        dimensions=["site_id"],
        filters=[SemanticFilter(field="site_id", operator="eq", value="site-1")],
        policy_registry=policy,
    )
    ...
    assert compiled.scoped_query.parameters["scope_0_0"] == "site-1"
