...
def _authorization(site_id="site-1", permission_version="1"):
...
async def test_action_requires_scope_approval_and_is_idempotent(repository):
    ...
    preview = await repository.preview(
        first["proposal_id"],
        authorization=authorization,
        target_snapshot={"id": "site-1", "version": "3"},
    )
    ...
    approved = await repository.approve(first["proposal_id"], authorization=authorization, approved_by="user-1")
...
async def test_worker_claim_and_finish_updates_execution(repository):
    ...
    await repository.preview(
        proposal["proposal_id"],
        authorization=authorization,
        target_snapshot={"id": "site-1"},
    )
    await repository.approve(proposal["proposal_id"], authorization=authorization, approved_by="user-1")
...
class _StaticTargetReader:
    def __init__(self, snapshot=None):
        self.snapshot = snapshot or {"id": "site-1"}
        self.calls = []
...
async def test_action_worker_rechecks_target_version_before_write(repository):
    ...
    worker = ActionWorker(
        ...
        executor=executor,
        authorization_revalidator=_StaticRevalidator(authorization),
        target_reader=_StaticTargetReader({"id": "site-1", "version": "4"}),
...
async def test_action_worker_runs_only_declared_compensation(tmp_path):
    ...
    try:
        ...
        assert completed["result"] == {"compensated": "site-1"}
