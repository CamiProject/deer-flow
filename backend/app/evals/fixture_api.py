...
class ResetRequest(BaseModel):
    ...
    tenant_id: str
    tenant_code: str
    tenant_name: str = "DeerFlow Eval Tenant"
    principal_id: str
    system_code: str
...
def _seed_state(scenario: str) -> dict[str, Any]:
    site_count = 2 if scenario == "two_visible_one_hidden" else 1
    sites = {
        f"site-{index}": {
            "id": f"site-{index}",
...
def create_fixture_app(*, settings: EvalFixtureSettings | None = None) -> FastAPI:
    ...
    app = FastAPI(title="DeerFlow Evals Fixture", version="1")
