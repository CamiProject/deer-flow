...
class EvalFixture(StrictModel):
    ...
    def require_eval_identity(self) -> EvalFixture:
        if not self.tenant_id.startswith("eval-") or not self.principal_id.startswith("eval-"):
            raise ValueError("evaluation fixtures must use eval-prefixed tenant and principal identities")
        return self
