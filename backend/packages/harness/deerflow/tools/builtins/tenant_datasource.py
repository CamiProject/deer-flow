"""Resolve SaaS tenant data sources from ``conf_database``."""
...
def build_database_code(ctx: TenantContext) -> str:
    ...
    return f"carbon_client_{ctx.system_code}_{ctx.tenant_code}"


def decrypt_password(value: str) -> str:
    """Hook for future encrypted ``conf_database.password`` support."""
    return value
...
def _query_conf_database(database_code: str) -> dict[str, str]:
    ...
    if not rows:
        raise TenantDataSourceError(f"No datasource found in conf_database for code {database_code!r}")
    row = rows[0]
    return {
        "code": str(row[0] or ""),
        "url": str(row[1] or ""),
...
def _resolve_by_code(database_code: str) -> TenantDataSource:
    row = _query_conf_database(database_code)
...
def resolve_tenant_datasource(ctx: TenantContext) -> TenantDataSource:
    """Resolve the current SaaS tenant datasource from ``conf_database``."""
